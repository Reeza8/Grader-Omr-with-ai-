...
from User.models import User, VerifyCode, UserRole
...
from utils.utils import generate_random_password, my_response, is_valid_email, is_valid_phone


router = APIRouter(prefix='/userApi')
...
async def login_teacher(
    ...
    if not user:
        user_data = {"name": "", "password_hash": "", "role": UserRole.teacher}
        if is_phone:
    ...
    res_data = LoginResponse(index=index, role=UserRole.teacher)
...
async def login_password(
    ...
    token = create_access_token(db_user.id, db_user.role.value, db_user.name)
